...
REPO_ROOT=$PWD
CONFIG_FILE="$REPO_ROOT/configuration.ini"
...
cd $RTA_DIR
python3 run.py --runtime-dir $RUNTIME_INSTALLATION_DIR --runtime-nomig-dir $RUNTIME_NO_MIG_INSTALLATION_DIR --util-low $1 --util-high $2 --util-step $3 --criticality-low $4 --criticality-high $5 --criticality-step $6 --proportion-low $7 --proportion-high $8 --proportion-step $9 --exp-1 ${10} --exp-2 ${11} --exp-3 ${12} --exp-4 ${13} --numb-tests ${14} --config-path $CONFIG_FILE
